          ...
          <tr v-for="(account, index) in accounts.data" :key="index">
            ...
            <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
              <span
                ...
                v-for="role in account.corporateRoles"
                :key="role._id"
              >
                {{ role.roleName }}</span
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="py-1">
                        <p
                          @click="resetPassword(account._id)"
                          class="flex items-center px-4 py-2 space-x-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"
                          role="menuitem"
                        >
          ...
          <tr>
            ...
            >
              ...
              <div v-else class="inline-block">
                ...
                <span class="block">No account found!</span>
              </div>
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  methods: {
    ...
    getAccounts() {
      this.loading = true;
      var path =
        'accounts?populate=["corporate"]&page=' +
        this.currentPage +
        "limit=" +
        this.perPage +
        "&corporate=" +
    ...
    edit(id) {
      var account = this.accounts.data.find((account) => account._id === id);
      this.changePage("edit", account);
    },
    resetPassword(id) {
      var account = this.accounts.data.find((account) => account._id === id);
      this.changePage("reset-password", account);
    },
    clickAwayHandler(e) {
      if (

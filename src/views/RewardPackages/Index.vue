  ...
  <div>
    <div class="space-y-4">
      <div class="flex justify-between">
        <h2 class="font-semibold text-gray-600 uppercase">Reward Packages</h2>
        <router-link
          to="/reward-packages/add"
      ...
      <div class="inline-block min-w-full mt-4 rounded-lg shadow">
        <table class="min-w-full leading-normal">
          ...
          <tbody class="relative" v-if="rewardPackages.length">
            <tr
              v-for="(rewardPackage, index) in rewardPackages"
              :key="rewardPackage._id"
            >
              ...
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <div class="flex gap-2">
                  <div
                    class="flex items-center justify-center block w-10 h-10 bg-gray-200 bg-cover border border-gray-500 rounded-full"
                    v-for="prize in rewardPackage.prizes"
                    :key="prize._id"
                    :title="prize.prizeItem"
                    :style="{ backgroundImage: `url('${prize.prizeImage}')` }"
                  ></div>
                </div>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ rewardPackage.minimumWalletAmountToQualify }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ rewardPackage.createdAt }}
                </p>
              </td>
              <td
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <div class="py-1">
                          <router-link
                            :to="`reward-packages/${rewardPackage._id}/edit`"
                        ...
                        <div class="py-1">
                          <button
                            @click="deletePackage(rewardPackage)"
                            class="flex items-center px-4 py-2 space-x-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"
                            role="menuitem"
                          >
...
export default {
  ...
  watch: {
    currentPage() {
      this.getRewardPackages();
    },
    perPage() {
      this.currentPage === 1 ? this.getRewardPackages() : (this.currentPage = 1);
    },
    selectedFilter() {
      this.getRewardPackages();
    },
  ...
  methods: {
    ...
    getRewardPackages() {
      ...
      this.$misc
        .get(`reward-packages`)
        .then((response) => {
          this.rewardPackages = response.data.data;
          this.paginator = {
    ...
    deletePackage(rewardPackage) {
      this.$misc
        .delete(`reward-packages/${rewardPackage._id}`)
        .then(() => {
          this.getRewardPackages()
          window.flash({
            type: "success",
            body: "Reward Package deleted successfully!",
          });
        })
        .catch((err) => console.log(err))
        .finally(() => (this.loading = false));

...
localization: Object
  ...
  watch: {
    localization: {
      ...
      handler() {
        this.$misc
      .get(`/driver-localizations/${this.localization.name}`)
      .then((response) => {
  ...
  methods: {
    update() {
      this.busy = true;
      this.$misc
        .patch(`/driver-localizations/${this.localization._id}`, { ...this.localizations })
        .then(() => {

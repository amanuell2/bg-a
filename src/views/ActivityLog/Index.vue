<template>
  <div>
    <div class="space-y-4">
      <div>
        <h2 class="font-semibold text-gray-600 uppercase">Activity Logs</h2>
      </div>
          ...
          <div class="flex">
            <div class="">
              ...
              >
                ...
                <option
                  v-for="type in activityLogTypes"
            ...
            <div class="ml-5">
              <span class="text-gray-700">Actor</span>
              <select
                class="w-full mt-1 form-select"
                v-model="selectedActorType"
              >
                <option :selected="selectedActorType === 'all'" value="all">
                  All
                </option>
                <option
                  v-for="actorType in [{value: 'accounts', label: 'Account'}, { value: 'Drivers', label: 'Drivers'}, { value: 'Users', label: 'Passengers'} ]"
                  :key="`actor-type-${actorType.value}`"
                  :selected="selectedActorType === actorType.value"
           ...
           <div class="relative ml-5" v-if="selectedActorType === 'Drivers'">
              <label class="block">
            ...
            <div class="relative ml-5" v-else-if="selectedActorType === 'Users'">
              <label class="block">
            ...
            <div class="relative ml-5" v-else-if="selectedActorType === 'accounts'">
              <label class="block">
                <span class="text-gray-700">User</span>
                <input
            ...
            <!-- <div class="relative ml-5">
              <label class="block">
                <span class="text-gray-700">Log Filter</span>
                <input
                  v-model="logFilter"
                  class="block w-full mt-1 form-input"
                  placeholder="Filter by log text..."
                />
              </label>
            </div> -->
      ...
      <div class="inline-block min-w-full mt-4 rounded-lg shadow">
        <table class="min-w-full leading-normal">
          ...
          <tbody class="relative" v-if="activityLogs.length">
            <tr v-for="activityLog in activityLogs" :key="activityLog._id">
              ...
              <td class="gap-2 px-5 py-5 text-sm bg-white">
                {{ activityLog.subjectModel }} - {{ activityLog.subject }}
              </td>
              <td class="gap-2 px-5 py-5 text-sm bg-white">
                <template v-if="activityLog.url">
                  {{ activityLog.url }}
                </template>
                <template v-else-if="activityLog.resource">
                  {{ activityLog.resourceModel }} - {{ activityLog.resource }}
                </template>
                <template v-else>
                  -
                </template>
            ...
            <tr>
              ...
              >
                ...
                <div v-else class="inline-block">
                  ...
                  <span class="block">No Activity Logs Found!</span>
                </div>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  watch: {
    ...
    perPage() {
      this.currentPage === 1 ? this.getActivityLogs() : (this.currentPage = 1);
    },
    range: {
      handler() {
        this.getActivityLogs();
...
this.$misc.get('/activity-logs/types').then(activityLogTypes => {
  this.activityLogTypes = activityLogTypes.data
})
...
this.perPage = 50
  ...
  methods: {
    ...
    deleteBan(banId) {
      this.$misc.delete(`bans/${banId}`).then(() => {
        this.getActivityLogs();
      });
    },
    ...
    getActivityLogs() {
      this.loading = true;
      var path = `activity-logs?limit=${this.perPage}&page=${this.currentPage}`;

      if (this.selectedActorType != 'all') {
        path += "&actor=" + this.selectedActorType;
      }
      
      if (this.selectedActorType === 'Drivers' && this.driverId) {
        path += "&actorId=" + this.driverId;
      }

      if (this.selectedActorType === 'Users' && this.passengerId) {
        path += "&actorId=" + this.passengerId;
      }

      if (this.selectedActorType === 'accounts' && this.userId) {
        path += "&actorId=" + this.userId;
      }

      // if (this.logFilter) {
      //   path += "&log=" + this.logFilter
      // }
      
      if (this.selectedFilter && this.selectedFilter != "all") {
        path += "&actionCode=" + this.selectedFilter;
      }

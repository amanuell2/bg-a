    ...
    <div class="space-y-4">
      <div class="flex items-end justify-between w-full pb-2">
        ...
        <div class="flex items-center h-full">
          ...
          <button
            @click="cancelTrip"
            v-else-if="trip && trip.status != 'CANCELLED'"
            class="w-20 h-10 mt-5 text-white rounded shadow-md bg-shuufare-600 hover:bg-shuufare-400"
          >
            Cancel
          </button>
      ...
      <div class="inline-block min-w-full mt-4 -lg" v-if="!loading">
        <div class="flex py-2 my-2" v-if="trip != null">
          ...
          <div class="w-1/6">
            ...
            <div v-if="trip.dispatcherWhoEnded">
              <p>
                {{ trip.dispatcherWhoEnded.firstName + " " + trip.dispatcherWhoEnded.lastName }}
              </p>
            </div>
            <div v-else-if="trip.cancelledBy">
              {{ trip.cancelledBy }}
            </div>
          </div>
        </div>
        ...
        <div class="flex py-2 my-2" v-if="trip != null">
          <div class="w-1/6">
            ...
            <div>
              <p>{{ trip.corporate ? trip.corporate.name : false }}</p>
            </div>
          </div>
          <div class="w-1/6">
          ...
          <div class="w-1/6">
            ...
            <div>
              <p>{{ trip.ticket ? trip.ticket.code : "" }}</p>
            </div>
          </div>
          <div class="w-1/6">
...
export default {
  ...
  methods: {
    ...
    getRide() {
      this.loading = true;
      var path = `trips/${this.id}?limit=populate=["driver","passenger","vehicleType","dispatcher", "corporate"]&start=${this.startDate}&end=${this.endDate}`;
      this.$misc
        .get(path)
        .then((response) => {
          this.trip = response.data;
          this.trip.route.coordinates = polyline.decode(this.trip.route.polyline)
          console.log(this.trip);
          this.totalTripDistance()

          this.route1 = this.trip.route.coordinates.map(x=>[x[1], x[0]])
          this.route2 = this.trip.path
          ...
          if (this.trip.path && this.trip.path.length > 1) {
            ...
            new this.google.maps.Marker({
              position: {
                lat: this.trip.path[this.trip.path.length - 1][0],
                lng: this.trip.path[this.trip.path.length - 1][1],
              },
              icon:
          ...
          new this.google.maps.Marker({
            position: {
              lat: this.trip.pickUpAddress.lat,
              lng: this.trip.pickUpAddress.long,
            },
            icon:
          ...
          new this.google.maps.Marker({
            position: {
              lat: this.trip.dropOffAddress.lat,
              lng: this.trip.dropOffAddress.long,
            },
            icon:
            ...
            .then((sos) => {
              if (sos.data && sos.data.length) {
                ...
                sos.data.forEach((element) => {
                  new this.google.maps.Marker({
                    position: {
                      lat: element.position.coordinates[0],
                      lng: element.position.coordinates[1],
                    },
                    icon,
                    map: this.map,
                  });

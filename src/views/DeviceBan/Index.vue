<template>
  <div>
    <div class="space-y-4">
      <div>
        <h2 class="font-semibold text-gray-600 uppercase">Device Bans</h2>
      </div>
            ...
            <tr v-for="(banModel, index) in banModels" :key="banModel._id">
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ banModel.deviceID ? banModel.deviceID : " - " }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
              ...
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ banModel.deviceModelID }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ banModel.allDevicesOfTheModel }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
...
export default {
  ...
  watch: {
    ...
    selectedDeviceID(searchQuery) {
      if (searchQuery) this.getDeviceBans();
    },
    selectedDeviceModelID() {
      this.getDeviceBans();
    },
    perPage() {
      this.currentPage === 1 ? this.getDeviceBans() : (this.currentPage = 1);
    },
    selectedFilter() {
      this.getDeviceBans();
    },
  ...
  methods: {
    ...
    getDeviceBans() {
      this.loading = true;
      var path = `bans?limit=${this.perPage}&page=${this.currentPage}`;

      if (this.selectedFilter != "all") {
        path += "&allDevicesOfTheModel=" + this.selectedFilter;
      }

      if (this.selectedDeviceModelID) {
        path += "&selectedDeviceModelID=" + this.selectedDeviceModelID;
      }

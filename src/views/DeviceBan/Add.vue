<template>
  <div>
    ...
    <form @submit.prevent class="space-y-4">
      ...
      <div>
        <photo-uploader
          v-model="driver.businessLicense"
          label="BusinessLicense"
        />
      </div>
      <div>
        <photo-uploader
          v-model="driver.drivingLicense"
      ...
      <div>
        ...
        <div class="flex gap-6">
          <photo-uploader
            v-for="(i, index) in driver.additionalDocuments" :key="`additional-document-${index}`"
            :value="driver.additionalDocuments[index]"
            @input="driver.additionalDocuments.splice(index, 1)"
          />
          <photo-uploader
            @input="x => driver.additionalDocuments.push(x)"
          />
        </div>
      </div>
      <label class="flex items-center space-x-4">
        <input
          type="checkbox"
          class="form-checkbox"
          v-model="driver.representative"
        />
        <span>Representative (Optional)</span>
      </label>
      <div v-if="driver.representative">
        <photo-uploader
          v-model="driver.representationPaper"
          label="Representation Paper"
        />
      </div>

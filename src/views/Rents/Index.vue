<template>
  <div>
    <div class="space-y-4">
      <div>
        <h2 class="font-semibold text-gray-600 uppercase">Rents</h2>
      </div>
          ...
          <div class="flex">
            <div class="">
              ...
              >
                ...
                <option
                  :selected="selectedFilter === 'STARTED'"
                  value="Started"
                >
                  Started
                </option>
      ...
      <div class="inline-block min-w-full mt-4 rounded-lg shadow">
        <table class="min-w-full leading-normal">
          ...
          <tbody class="relative" v-if="rents.length">
            <tr v-for="rent in rents" :key="rent._id">
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{
                    rent.dispatcher
                      ? rent.dispatcher.firstName +
                        " " +
                        rent.dispatcher.lastName
                      : " - "
                  }}
                </p>
              </td>
              ...
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{
                    rent.driver
                      ? rent.driver.firstName + " " + rent.driver.lastName
                      : "Unknown"
                  }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{
                    rent.startTimestamp
                      ? formatDate(rent.startTimestamp)
                      : "-"
                  }}
                </p>
                <p class="text-gray-900 whitespace-no-wrap">
              ...
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <div>
                  <div class="flex items-center space-x-2">
                    ...
                    <span>{{ rent.pickUpAddress.name }}</span>
                  </div>
                </div>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">{{ rent.fare.toFixed(2) }} <span v-if="rent.discount">({{rent.discount}} discount)</span></p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  {{ rent.vehicleType ? rent.vehicleType.name : "-" }}
                </p>
              </td>
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
              ...
              <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                <p class="text-gray-900 whitespace-no-wrap">
                  <a :href="'/rents/' + rent._id + '/show'" target="self"
            ...
            <tr>
              ...
              >
                ...
                <div v-else class="inline-block">
                  ...
                  <span class="block">No latest rents!</span>
                </div>
              </td>
            </tr>
          </tbody>
  ...
  methods: {
    ...
    searchPassenger(e) {
      if (e.target.value.trim() === "") {
         ...
         this.selectedPassengerObj = null;
        this.selectedPassenger = '';
        this.passengerSearchResults = [];
        this.passengerId = null;
        const query = { ...this.$route.query, passenger: null };
        this.$router.push({ query });
    ...
    getRides() {
      this.loading = true;
      var path = `rents?limit=${this.perPage}&page=${this.currentPage}&populate=["driver","passenger","vehicleType","dispatcher"]`;

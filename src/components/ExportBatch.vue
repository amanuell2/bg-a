<template>
  <div>
    ...
    <button
      v-if="$store.getters.role && $store.getters.role.permissions.reports && $store.getters.role.permissions.reports.canAccess"
...
export default {
  ...
  methods: {
    async downloadReport() {
      ...
      const data = []
      alert(JSON.stringify(this.filters, null, 2))

      for (const report of this.appModules.filter(({ value }) => value)) {
        try {

          this.$emit('starting', report)

          const result = await this.$misc({
            url: [config.baseURL, report.exportUrl].join("/") + '?' + (
              Object.entries({
              ...Object.fromEntries(
                Object.entries({
...
,
            method: "GET",
            data: {
              // appModules: this.appModules.map(({ fieldName, value }) => ({ fieldName, value }))
            },
            responseType: "text",

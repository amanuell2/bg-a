<template>
  <div>
    <button
      v-if="!disabled && $store.getters.role.permissions.reports && $store.getters.role.permissions.reports.canAccess"
    ...
    >
      ...
      <div class="flex gap-2" v-else>
        ...
        <template v-else>  
          Export to {{ downloaded ? `${format} (again)` : format }}
        </template>
      </div>
      <div></div>
    </button>
...
export default {
  ...
  methods: {
    downloadReport() {
      ...
      return this.$misc({
        url: [config.baseURL, this.downloadUrl].join("/"),
        method: "GET",
        responseType: "blob",
        ...
        setTimeout(() => {
          ...
          link.setAttribute(
            "download",
            [this.filename, this.format.toLowerCase()].join(".")
          );

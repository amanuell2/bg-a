<template>
  <div>
    <label class="block">
      <span class="text-gray-700 aria-label">{{ label }}</span>
      <p v-if="showError" v-text="error" class="text-red-500"></p>
      <input class="hidden block" type="file" @change="upload" />
      <div
        class="relative w-48 h-48 mt-1 bg-gray-200 border-4 rounded-lg"
        :class="{
          'border-gray-500': !showError,
          'border-red-500': showError,
          'border-dashed': !value,
        }"
      >
        <button class="absolute top-0 right-0" v-show="value" @click="reset">
          <i
            class="text-white bg-red-500 border-2 rounded-full shadow material-icons hover:bg-red-400"
          >
            close
          </i>
        </button>
        <div v-if="value && !uploading" class="w-full h-full">
          <div
            v-if="value.endsWith('.pdf')"
          ...
          >
              ...
              <metadata id="metadata5">
                <rdf:RDF>
                  <cc:Work rdf:about="">
                    ...
                    <dc:description
                      >Fuente del texto &quot;PDF&quot;: Franklin Gothic Medium
                      Cond</dc:description
                    >
                    <cc:license
                    ...
                    <g id="g858">
                      <path
                        ...
                        style="
                          ...
                          letter-spacing: 0px;
                          word-spacing: 4.26000023px;
                          fill: #2c2c2c;
                          fill-opacity: 1;
                          stroke: none;
                          stroke-width: 0.35824656px;
                          stroke-linecap: butt;
            ...
            <button class="px-4 py-2 pt-2 mt-3 font-semibold text-gray-800 border-2 rounded-lg hover:bg-shuufare-400 border-shuufare-500 hover:border-2 hover:text-gray-700" @click="downloadPDF">
              {{ downloading ? 'Downloading ...' : 'Download File' }}
            </button>
          </div>

          <img v-else class="object-cover w-full h-full rounded" :src="value" />
        </div>

        <div v-else class="flex items-center justify-center w-full h-full">
          <i
            v-if="uploading"
            class="text-6xl text-gray-500 material-icons spin"
          >
            sync
          </i>
          <i v-else class="text-6xl text-gray-500 material-icons">
            add_circle_outline
          </i>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      uploadUrl: "https://api.cloudinary.com/v1_1/dwh3txcll/upload",
      uploadPreset: "iliftpreset1ERPPOOUU",
      uploading: false,
      showError: false,
      downloaded: false,
      downloading: false,
